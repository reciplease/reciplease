...
class OpenFoodFactsAdapterTest {
    ...
    void searchByNameMapsProducts() {
        mockServer
                .expect(requestTo(org.hamcrest.Matchers.startsWith(
                        "https://world.openfoodfacts.org/cgi/search.pl?search_terms=yogurt")))
        ...
        assertThat(
                results,
                contains(new FoodCatalogEntry("123", "Greek Yogurt", "Chobani", new Nutrients(120.0, 10.0, 3.0, 5.0))));
    }
    ...
    void searchByNameSkipsUnnamedProductsAndToleratesFailure() {
        mockServer
                .expect(requestTo(org.hamcrest.Matchers.startsWith("https://world.openfoodfacts.org/cgi/search.pl")))
                .andRespond(withSuccess("""
    ...
    void lookupByBarcodeReturnsProduct() {
        mockServer
                .expect(
                        requestTo(
                                "https://world.openfoodfacts.org/api/v2/product/012345?fields=code,product_name,brands,nutriments"))
                .andRespond(withSuccess("""
                        {"status": 1, "product": {"code": "012345", "product_name": "Oat Milk", "brands": "Oatly",
                         "nutriments": {"energy-kcal_100g": 45.0, "proteins_100g": 1.0, "fat_100g": 1.5, "carbohydrates_100g": 6.0}}}""", MediaType.APPLICATION_JSON));

        final var result = openFoodFactsAdapter.lookupByBarcode("012345");

        assertThat(
                result,
                is(java.util.Optional.of(
                        new FoodCatalogEntry("012345", "Oat Milk", "Oatly", new Nutrients(45.0, 1.0, 1.5, 6.0)))));
    }
    ...
    void lookupByBarcodeReturnsEmptyWhenMissingOrFailing() {
        mockServer
                .expect(
                        requestTo(
                                "https://world.openfoodfacts.org/api/v2/product/000000?fields=code,product_name,brands,nutriments"))
                .andRespond(withSuccess("""
        ...
        mockServer
                .expect(
                        requestTo(
                                "https://world.openfoodfacts.org/api/v2/product/000001?fields=code,product_name,brands,nutriments"))
                .andRespond(withServerError());
