...
class PantryServiceTest {
    ...
    void setUp() {
        now = Instant.now();
        pantryService = new PantryService(pantryRepository, Clock.fixed(now, ZoneOffset.UTC));
    }
    ...
    void save() {
        ...
        when(pantryRepository.save(item)).thenReturn(savedItem);

        var actual = pantryService.save(item);
    ...
    void noItem() {
        ...
        var item = pantryService.findById(itemId);
    ...
    class WithItem {
        ...
        void findById() {
            when(pantryRepository.findById(item.id())).thenReturn(Optional.of(item));

            var actual = pantryService.findById(item.id());
        ...
        void findAll() {
            ...
            when(pantryRepository.betweenDates(HOUSE_ID, today)).thenReturn(List.of(expiredItem));

            var actual = pantryService.findAll(HOUSE_ID);
        ...
        void findAllIncludesFullyConsumedByDefault() {
            ...
            var today = now.atOffset(ZoneOffset.UTC).toLocalDate();
            when(pantryRepository.expiresAfter(HOUSE_ID, today)).thenReturn(List.of(consumedItem));
            when(pantryRepository.betweenDates(HOUSE_ID, today)).thenReturn(List.of());

            var actual = pantryService.findAll(HOUSE_ID, false);
        ...
        void findAllCanExcludeFullyConsumed() {
            var consumedItem = item.withRemaining(0d);
            var today = now.atOffset(ZoneOffset.UTC).toLocalDate();
            when(pantryRepository.expiresAfter(HOUSE_ID, today)).thenReturn(List.of(item, consumedItem));
            when(pantryRepository.betweenDates(HOUSE_ID, today)).thenReturn(List.of());

            var actual = pantryService.findAll(HOUSE_ID, true);
        ...
        void shouldUpdatePreservingAuditFields() {
            ...
            var updated = pantryService.update(item.id(), updates).orElseThrow();
        ...
        void shouldArchiveWhenUpdateZerosRemaining() {
            ...
            var updated = pantryService.update(item.id(), updates);

            assertThat(updated.isEmpty(), is(true));
            verify(pantryRepository).deleteById(item.id());
    ...
    void shouldDeleteById() {
        ...
        pantryService.deleteById(itemId);

        verify(pantryRepository).deleteById(itemId);
    }
    ...
    class Consume {
        ...
        void reducesRemaining() {
            ...
            var updated = pantryService.consume(existing.id(), 3d).orElseThrow();
        ...
        void clampsAtZeroAndArchives() {
            ...
            var updated = pantryService.consume(existing.id(), 5d);

            assertThat(updated.isEmpty(), is(true));
            verify(pantryRepository).deleteById(existing.id());
            verify(pantryRepository, never()).save(any());
        }
        ...
        void shouldThrowWhenItemMissing() {
            ...
            assertThrows(IllegalArgumentException.class, () -> pantryService.consume(itemId, 1d));
