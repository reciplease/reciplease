...
class FoodSearchServiceTest {
    ...
    void searchMergesHistoryAndCatalog() {
        ...
        when(foodConsumptionLoggerPort.history(USER_ID))
                .thenReturn(List.of(
                        new LoggedFoodHistoryEntry("food-1", "Chicken Breast, grilled"),
                        new LoggedFoodHistoryEntry(null, "Homemade Soup")));
        when(foodCatalogPort.searchByName("chick"))
                .thenReturn(List.of(new FoodCatalogEntry(
                        "123", "Chicken Tikka Masala", "Trader Joe's", new Nutrients(200.0, 15.0, 8.0, 10.0))));
    ...
    void searchSkipsHistoryWhenNotConnected() {
        ...
        when(foodCatalogPort.searchByName("yogurt"))
                .thenReturn(List.of(new FoodCatalogEntry("1", "Greek Yogurt", "Chobani", null)));
    ...
    void fuzzyMatchIsCaseInsensitiveAndRequiresEveryWord() {
        ...
        assertThat(
                results,
                contains(new FoodSearchResult(FoodSearchResultSource.HISTORY, "Greek Yogurt, Plain", "food-1", null)));
    }

    @Test
    @DisplayName("searchByBarcode delegates to the catalog port only")
    void searchByBarcodeDelegatesToCatalogOnly() {
        when(foodCatalogPort.lookupByBarcode("012345"))
        ...
        assertThat(
                result,
                is(Optional.of(new FoodSearchResult(
