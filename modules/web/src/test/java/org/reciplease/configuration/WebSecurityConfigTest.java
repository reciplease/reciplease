...
@SpringBootTest(
        classes = {
            ...
            WebSecurityConfigTest.TestController.class
        },
...
class WebSecurityConfigTest {
    ...
    void csrfIsDisabled() throws Exception {
        mockMvc.perform(post("/api/test")).andExpect(status().isCreated());
    }

    @Test
    @DisplayName("does not create a session")
    void sessionsAreStateless() throws Exception {
        mockMvc.perform(get("/api/measures")).andExpect(status().isOk()).andExpect(cookie().doesNotExist("JSESSIONID"));
    }
