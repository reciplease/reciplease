Feature: House invites and role-based access
  ...
  Background:
    Given a house "Bayview Gardens" owned by "owner-1"
  ...
  Scenario: Accepting a valid invite allowlists the user and grants the invited role
    ...
    When "newcomer-1" with email "newcomer@example.com" accepts invite "INVITE-1"
    Then the response status is 200
    And "newcomer@example.com" is on the allowlist
  ...
  Scenario: An invite code can only be redeemed once
    ...
    And "newcomer-1" with email "newcomer@example.com" accepts invite "INVITE-2"
    When "newcomer-2" with email "newcomer2@example.com" accepts invite "INVITE-2"
  ...
  Scenario: A read-only member cannot create inventory
    ...
    And "member-1" with email "member@example.com" accepts invite "INVITE-3"
  ...
  Scenario: A read-only member can list inventory in their own house
    ...
    And "member-2" with email "member2@example.com" accepts invite "INVITE-4"
